...
export const actions = {
	...
	addAuthMethod: async ({ request }) => {
		...
		} else if (type === "oauth2_service_account") {
			const saJson = data.get("saJsonCredential")?.toString() ?? "";
			const scopes = data.get("saScopes")?.toString()?.trim() ?? "";
			if (!saJson) return fail(400, { error: "Service account JSON is required" });

			try {
				const parsed = JSON.parse(saJson);
				if (!parsed.client_email || !parsed.private_key) {
					return fail(400, { error: "Invalid service account JSON: missing client_email or private_key" });
				}
				parsed.scopes = scopes || "https://www.googleapis.com/auth/devstorage.read_only";
