...
import { apiDownload } from "./tools/api-download";
...
export function registerTools(server: McpServer, token: Token) {
	...
	server.tool(
		"api_download",
		"Download an image response from an API target through Shellgate as base64. Use for authenticated binary images such as Linear uploads. Only image/png, image/jpeg, and image/webp are accepted.",
		{
			target: z.string().describe("Target slug"),
			path: z.string().describe("Path appended to target's baseUrl"),
